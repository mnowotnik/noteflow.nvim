...
test: deps
	DEBUG_PLENARY=true nvim --headless --noplugin -u tests/minimal_init.vim -c "PlenaryBustedDirectory tests/plenary/ {minimal_init = 'tests/minimal_init.vim'}"
...
ci-test: github-actions-setup.sh
	bash -c 'source github-actions-setup.sh nightly-x64 && export DEBUG_PLENARY=true && make test'
